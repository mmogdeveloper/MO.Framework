...
namespace MO.Unity3d.Network.PacketHandler
{
    public class Action100001Handler : IPacketHandler
    {
        ...
        public void Handle(object sender, Packet packet)
        {
            ...
            foreach (var item in rep.UserTransforms)
            {
                if (GameUser.Instance.UserId == item.UserId)
                {
                    GameUser.Instance.CurPlayer.Position = new Vector3(item.Position.X, item.Position.Y, item.Position.Z);
                ...
                else
                {
                    if (!GameUser.Instance.Players.ContainsKey(item.UserId))
                    {
                        ...
                        newPlayer.Position = new Vector3(item.Position.X, item.Position.Y, item.Position.Z);
